...
# def GA_IRS(p_max): # Define function to process each p_max value
#     print(f"calculaiton for p_max for GA_IRS",p_max)
...
#                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     }
#                         if V_lm_hfly_value>0 and T_m_har_value>0 and T_ml_down_value>0 and T_km_up_value>0 and P_m_har_value<=p_max and P_m_down_value<=p_max and P_km_up_value<=p_km_max and (T_km_com_value+T_km_up_value+T_ml_down_value)<=T_m and f_km_value>0  and E_kml_har_value>=(E_kml_up_value+E_kml_com_value) and V_lm_vfly_value>0 and T_km_com_value>=T_m_har_value:
...
# def GA_IRS_RA(p_max): # Define function to process each p_max value
#     print(f"calculaiton for p_max for GA_IRS_RA",p_max)
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# def HC_IRS(p_max):
#     print(f"calculaiton for p_max for HC_IRS",p_max)
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# # Define a function to process each p_max value
# def HC_IRS_RA(p_max):
#     print(f"calculaiton for p_max for HC_IRS_RA",p_max)
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# Define a function to process each p_max value
def RS(p_max):
    print(f"calculaiton for p_max for RS",p_max) # corrected print statement
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        ...
        # fitness_sums_HC_IRS = pool.map(HC_IRS, p_max_values)
        # fitness_sums_HC_IRS_RA = pool.map(HC_IRS_RA, p_max_values)
        fitness_sums_RS = pool.map(RS, p_max_values)
    ...
    csv_file_path_pandas = "fitness_summary_Power(t).csv"
    ...
    p_max_values =np.arange(1, 11, 1)
    plt.rcParams["font.size"] = "20"
    # plt.plot(p_max_values, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    # plt.plot(p_max_values, fitness_sums_GA_IRS_RA, marker='s', linestyle='dotted',label = "C2G-A_IRS_RA")
    # plt.plot(p_max_values, fitness_sums_HC_IRS, marker='s', linestyle='-',label = "HC-A_IRS")
    # plt.plot(p_max_values, fitness_sums_HC_IRS_RA, marker='*', linestyle='-',label = "HC-A_IRS_RA")
    plt.plot(p_max_values, fitness_sums_RS, marker='o', linestyle='dashdot',label = "RS-A")
    plt.xlabel('Power',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    # plt.savefig("Energy vs Power(t).pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
