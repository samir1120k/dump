...
f_l_m_df=pd.read_csv(r'h_l_m.csv') # number of IRS is 500 store in each column # corrected filename
...
summed_fitness_per_generation_HC = [0] * num_generation # Initialize list to store summed fitness for each generation
...
for l in range(num_bs):
    ...
    for k in range(num_uav_irs):
        ...
        generations_data = []
        generation_fitness_values = [] # List to store fitness values for each generation
        for j in range(num_generation): # Hill Climbing iterations
            ...
            generation_fitness_values.append(current_solution['fitness']) # Store fitness value

        # After inner hill climbing loop for a BS-UAV pair
        if all_best_combinations: # Check if all_best_combinations is not empty
            for gen_index in range(num_generation): # Iterate through generations
                if len(all_best_combinations[-1]['generation_fitness']) > gen_index: # Check if gen_index is valid
                    if len(summed_fitness_per_generation_HC) <= gen_index: # Extend list if needed
                        summed_fitness_per_generation_HC.extend([0] * (gen_index - len(summed_fitness_per_generation_HC) + 1))
                    summed_fitness_per_generation_HC[gen_index] += all_best_combinations[-1]['generation_fitness'][gen_index] # Sum fitness for each generation
                else:
                    break # Exit inner loop if generation_fitness list is shorter than gen_index
        ...
        all_best_combinations.append({
            ...
            'generation_fitness': generation_fitness_values, # Store generation fitness values
...
# Plotting Sum of Best Fitness vs Generation
plt.figure(figsize=(12, 6))
generation_numbers = range(1, num_generation + 1)
plt.plot(generation_numbers, summed_fitness_per_generation_HC[:num_generation]) # Plot summed fitness
plt.xlabel('Generation')
plt.ylabel('Sum of Best Fitness Value')
plt.title('Sum of Best Fitness Value vs. Generation (Hill Climbing)')
plt.grid(True)
plt.show()

# Print the fitness sum of the last generation
if summed_fitness_per_generation_HC: # Check if the list is not empty
    last_generation_fitness_sum = summed_fitness_per_generation_HC[-1]
    print(f"\nSum of Fitness Value for the Last Generation: {last_generation_fitness_sum:.4f}")
else:
    print("\nNo fitness data available to calculate last generation sum.")