...
def GA_IRS(kappa): # Define function to process each p_max value
    print(f"calculaiton for kappa for GA_IRS",kappa)
...
def GA_IRS_RA(kappa): # Define function to process each p_max value
    print(f"calculaiton for kappa for GA_IRS_RA",kappa)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    def E_kml_har(P_m_har,T_m_har,h_km_har):
...
def HC_IRS(kappa):
    print(f"calculaiton for kappa for HC_IRS",kappa)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    def E_kml_har(P_m_har,T_m_har,h_km_har):
...
# Define a function to process each p_max value
def HC_IRS_RA(kappa):
    print(f"calculaiton for kappa for HC_IRS_RA",kappa)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    def E_kml_har(P_m_har,T_m_har,h_km_har):
...
# Define a function to process each p_max value
def RS(kappa):
    print(f"calculaiton for kappa for RS",kappa) # corrected print statement
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    def E_kml_har(P_m_har,T_m_har,h_km_har):
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    kappa = np.arange(0.1, 1.1, 0.1) # P_max values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, kappa)
        fitness_sums_GA_IRS_RA = pool.map(GA_IRS_RA, kappa)
        fitness_sums_HC_IRS = pool.map(HC_IRS, kappa)
        fitness_sums_HC_IRS_RA = pool.map(HC_IRS_RA, kappa)
        fitness_sums_RS = pool.map(RS, kappa)
    ...
    csv_file_path_pandas = "fitness_summary Kappa.csv"
    ...
    kappa =np.arange(0.1, 1.1, 0.1)
    plt.rcParams["font.size"] = "20"
    plt.plot(kappa, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    plt.plot(kappa, fitness_sums_GA_IRS_RA, marker='s', linestyle='dotted',label = "C2G-A_IRS_RA")
    plt.plot(kappa, fitness_sums_HC_IRS, marker='s', linestyle='-',label = "HC-A_IRS")
    plt.plot(kappa, fitness_sums_HC_IRS_RA, marker='*', linestyle='-',label = "HC-A_IRS_RA")
    plt.plot(kappa, fitness_sums_RS, marker='o', linestyle='dashdot',label = "RS-A")
    plt.xlabel('Kappa(\u03BA)',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Kappa(\u03BA).pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
