...
num_bs = 5
num_irs_ele=50
num_generation = 20 # Number of generations, increased for GA to evolve
num_uav_irs = 8
...
def GA_IRS(T_m): # Define function to process each p_max value
    print(f"calculaiton for T_m for GA_IRS",T_m)
...
# def GA_IRS_RA(T_m): # Define function to process each p_max value
#     print(f"calculaiton for T_m for GA_IRS_RA",T_m)
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# def HC_IRS(T_m):
#     print(f"calculaiton for T_m for HC_IRS",T_m)
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# # Define a function to process each p_max value
# def HC_IRS_RA(T_m):
#     print(f"calculaiton for T_m for HC_IRS_RA",T_m)
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# # Define a function to process each p_max value
# def RS(T_m):
#     print(f"calculaiton for T_m for RS",T_m) # corrected print statement
#     all_best_combinations = []
#     all_best_individuals = []
#     sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
#                     elif key in ['P_km_up_value','P_m_down_value', 'P_m_har_value', 'T_m_har_value','f_km_value']:
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, T_m_values)
        # fitness_sums_GA_IRS_RA = pool.map(GA_IRS_RA, T_m_values)
        # # fitness_sums_HC_IRS = pool.map(HC_IRS, T_m_values)
        # fitness_sums_HC_IRS_RA = pool.map(HC_IRS_RA, T_m_values)
        # fitness_sums_RS = pool.map(RS, T_m_values)
        

    data_dict = {
        "Time Delay": T_m_values, # Assuming generation number starts from 1
    ...
    csv_file_path_pandas = "fitness_summary Time Delay(25 PS try).csv"
    ...
    plt.plot(T_m_values, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    # plt.plot(T_m_values, fitness_sums_GA_IRS_RA, marker='s', linestyle='dotted',label = "C2G-A_IRS_RA")
    # plt.plot(T_m_values, fitness_sums_HC_IRS, marker='s', linestyle='-',label = "HC-A_IRS")
    # plt.plot(T_m_values, fitness_sums_HC_IRS_RA, marker='*', linestyle='-',label = "HC-A_IRS_RA")
    # plt.plot(T_m_values, fitness_sums_RS, marker='o', linestyle='dashdot',label = "RS-A")
    plt.xlabel('Time Delay',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Time Delay(25 PS try).pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
    ...
    # new_row = {"Time Delay":"Fitness Improvement(%)", "Fitness_Sum_HC_IRS_RA":improvement_GA_IRS_vs_HC_IRS_RA, "Fitness_Sum_GA_IRS_RA":improvement_GA_IRS_vs_GA_IRS_RA, "Fitness_Sum_GA_IRS":0, "Fitness_Sum_RS":improvement_GA_IRS_vs_RS}
