...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    data_dict = {
        "Time Delay": list(range(1, len(fitness_sums_GA_IRS) + 1)) if fitness_sums_GA_IRS else [], # Assuming generation number starts from 1
    ...
    plt.legend()
    plt.savefig("Energy vs Time Delay.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
