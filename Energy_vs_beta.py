...
def GA_IRS(beta): # Define function to process each p_max value
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, beta_value)
    ...
    csv_file_path_pandas = "fitness_summary Beta.csv"
    ...
    plt.plot(beta_value, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    plt.xlabel('p_mutation',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Beta.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
