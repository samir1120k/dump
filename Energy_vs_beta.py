...
Km25= [] # Store sum of fitness values for each p_max


def km25(beta): # Define function to process each p_max value
    print(f"calculaiton for beta for GA_IRS",beta)
    population_size =25
                        ...
                        if key in numerical_keys_for_crossover:
                            if key in ['Angle1_row','Angle_row','Angle2_row']: # Handle Angle Series
                                ...
                                for col in Angle_df.columns: # Iterate through each column (angle direction)
                                    child_data[key][col] = float(parent1['data'][key][col]) * beta + float(parent2['data'][key][col]) * (1 - beta)
                            else: # Handle other numerical values as before
                                child_data[key] = float(parent1['data'][key]) * beta + float(parent2['data'][key]) * (1 - beta)
                        else:
                            child_data[key] = float(parent1['data'][key]) * beta + float(parent2['data'][key]) * (1 - beta)
...
Km30= [] # Store sum of fitness values for each p_max


def km30(beta): # Define function to process each p_max value
    print(f"calculaiton for beta for GA_IRS",beta)
    population_size =30
    all_best_combinations = []
    all_best_individuals = []
...
Km35= [] # Store sum of fitness values for each p_max


def km35(beta): # Define function to process each p_max value
    print(f"calculaiton for beta for GA_IRS",beta)
    population_size =35
    all_best_combinations = []
    all_best_individuals = []
...
Km40= [] # Store sum of fitness values for each p_max


def km40(beta): # Define function to process each p_max value
    print(f"calculaiton for beta for GA_IRS",beta)
    population_size =40
    all_best_combinations = []
    all_best_individuals = []
...
Km45= [] # Store sum of fitness values for each p_max


def km45(beta): # Define function to process each p_max value
    print(f"calculaiton for beta for GA_IRS",beta)
    population_size =45
    all_best_combinations = []
    all_best_individuals = []
...
Km50= [] # Store sum of fitness values for each p_max


def km50(beta): # Define function to process each p_max value
    print(f"calculaiton for beta for GA_IRS",beta)
    population_size =50
    all_best_combinations = []
    all_best_individuals = []
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    beta_value = np.arange(0.1,1,0.1) # P_max values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        Km25= pool.map(km25, beta_value)
        Km30 = pool.map(km30, beta_value)
        Km35 = pool.map(km35, beta_value)
        Km40 = pool.map(km40, beta_value)
        Km45 = pool.map(km45, beta_value)
        Km50 = pool.map(km50, beta_value)

    data_dict = {
        "Mutation_value": beta_value, # Assuming generation number starts from 1
        "Km_25": Km25,
        "Km_30": Km30,  
        "Km_35": Km35,  
        "Km_40": Km40,  
    ...
    csv_file_path_pandas = "fitness_summary Beta(km).csv"
    ...
    beta_value =np.arange(0.1,1,0.1)
    plt.rcParams["font.size"] = "20"
    plt.plot(beta_value, Km25, marker='*', linestyle='-',label = "Km=25")
    plt.plot(beta_value, Km30, marker='o', linestyle='-',label = "Km=30")
    plt.plot(beta_value, Km35, marker='.', linestyle='-',label = "Km=35")
    plt.plot(beta_value, Km40, marker='x', linestyle='-',label = "Km=40")
    plt.plot(beta_value, Km45, marker='X', linestyle='-',label = "Km=45")
    plt.plot(beta_value, Km50, marker='+', linestyle='-',label = "Km=50")
    plt.xlabel("\u03B2",size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Beta(km).pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
