...
def GA_IRS(B): # Define function to process each p_max value
    print(f"calculaiton for B for GA_IRS",B)
    ...
    for l in range(num_bs):
        ...
        for k in range(num_uav_irs):
            ...
            for j in range(num_generation):
                child_population = []
                # Corrected loop range to use valid_indices length
                for x in range(0, population_size, 2): # Loop through population with step of 2
                    if x + 1 >= population_size: # Check if i+1 is within bounds, if not break to avoid error in accessing population[i+1]
                        break
                    # Crossover
                    ranodmpopulation=[]
                    for i in range(10):
...
def GA_IRS_RA(B): # Define function to process each p_max value
    print(f"calculaiton for B for GA_IRS_RA",B)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
def HC_IRS(B):
    print(f"calculaiton for B for HC_IRS",B)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# Define a function to process each p_max value
def HC_IRS_RA(B):
    print(f"calculaiton for B for HC_IRS_RA",B)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# Define a function to process each p_max value
def RS(B):
    print(f"calculaiton for B for RS",B) # corrected print statement
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    Bandwidth = np.arange(1, 11,1) # P_max values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, Bandwidth)
        fitness_sums_GA_IRS_RA = pool.map(GA_IRS_RA, Bandwidth)
        fitness_sums_HC_IRS = pool.map(HC_IRS, Bandwidth)
        fitness_sums_HC_IRS_RA = pool.map(HC_IRS_RA, Bandwidth)
    ...
    csv_file_path_pandas = "fitness_summary Bandwidth(25 PS).csv"
    ...
    plt.plot(Bandwidth, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    plt.plot(Bandwidth, fitness_sums_GA_IRS_RA, marker='s', linestyle='dotted',label = "C2G-A_IRS_RA")
    plt.plot(Bandwidth, fitness_sums_HC_IRS, marker='s', linestyle='-',label = "HC-A_IRS")
    plt.plot(Bandwidth, fitness_sums_HC_IRS_RA, marker='*', linestyle='-',label = "HC-A_IRS_RA")
    plt.plot(Bandwidth, fitness_sums_RS, marker='o', linestyle='dashdot',label = "RS-A")
    plt.xlabel('Bandwidth',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Bandwidth(25 PS).pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
    ...
    new_row = {"Bandwidth":"Fitness Improvement", "fitness_sums_HC_IRS_RA":improvement_GA_IRS_vs_HC_IRS_RA, "fitness_sums_HC_IRS":improvement_GA_IRS_vs_HC_IRS, "fitness_sums_GA_IRS_RA":improvement_GA_IRS_vs_GA_IRS_RA, "fitness_sums_GA_IRS":0, "fitness_sums_RS":improvement_GA_IRS_vs_RS}
