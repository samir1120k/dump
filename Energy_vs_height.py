...
def GA_IRS(H_value): # Define function to process each p_max value
    print(f"calculaiton for H_value for GA_IRS",H_value)
...
def GA_IRS_RA(H_value): # Define function to process each p_max value
    print(f"calculaiton for H_value for GA_IRS_RA",H_value)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    Bh = (1 - 2.2558 * pow(10, -5) *H_value)**4.2577
...
def HC_IRS(H_value):
    print(f"calculaiton for H_value for HC_IRS",H_value)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    Bh = (1 - 2.2558 * pow(10, -5) *H_value)**4.2577
...
# Define a function to process each p_max value
def HC_IRS_RA(H_value):
    print(f"calculaiton for H_value for HC_IRS_RA",H_value)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    Bh = (1 - 2.2558 * pow(10, -5) *H_value)**4.2577
...
# Define a function to process each p_max value
def RS(H_value):
    print(f"calculaiton for H_value for RS",H_value) # corrected print statement
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
    Bh = (1 - 2.2558 * pow(10, -5) *H_value)**4.2577
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    height_values = np.arange(10, 51,5) # P_max values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, height_values)
        fitness_sums_GA_IRS_RA = pool.map(GA_IRS_RA, height_values)
        fitness_sums_HC_IRS = pool.map(HC_IRS, height_values)
        fitness_sums_HC_IRS_RA = pool.map(HC_IRS_RA, height_values)
        fitness_sums_RS = pool.map(RS, height_values)
    ...
    csv_file_path_pandas = "fitness_summary height(25 PS).csv"
    ...
    height_values =np.arange(10, 51, 5)
    plt.rcParams["font.size"] = "20"
    plt.plot(height_values, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    plt.plot(height_values, fitness_sums_GA_IRS_RA, marker='s', linestyle='dotted',label = "C2G-A_IRS_RA")
    plt.plot(height_values, fitness_sums_HC_IRS, marker='s', linestyle='-',label = "HC-A_IRS")
    plt.plot(height_values, fitness_sums_HC_IRS_RA, marker='*', linestyle='-',label = "HC-A_IRS_RA")
    plt.plot(height_values, fitness_sums_RS, marker='o', linestyle='dashdot',label = "RS-A")
    plt.xlabel('Height',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs Height(25 PS).pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
