...
def GA_IRS(num_irs_ele): # Define function to process each p_max value
    print(f"calculaiton for num_irs_ele for GA",num_irs_ele)
    ...
    for l in range(num_bs):
        ...
        for k in range(num_uav_irs):
            ...
            Sub_value=0
            # Calculate Sub_value for the current UAV and BS combination
            for i in range(len(valid_indices)):
                h_il_up_value=h_kml_down(Angle_UP_df.iloc[valid_indices[i], :],g_l_m_df.iloc[k, :],g_l_km_df_bs.iloc[i, :])
...
def GA_IRS_RA(num_irs_ele): # Define function to process each p_max value
    print(f"calculaiton for num_irs_ele for GA",num_irs_ele)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
def HC_IRS(num_irs_ele):
    print(f"calculaiton for num_irs_ele for HC",num_irs_ele)
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
# Define a function to process each p_max value
def RS(num_irs_ele):
    print(f"calculaiton for num_irs_ele for RS",num_irs_ele) # corrected print statement
    all_best_combinations = []
    all_best_individuals = []
    sum_fitness_current_p_max = 0 # Initialize sum of fitness for current p_max
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    num_irs = np.arange(10, 51, 5) # P_max values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, num_irs)
        fitness_sums_GA_IRS_RA = pool.map(GA_IRS_RA, num_irs)
        fitness_sums_HC_IRS = pool.map(HC_IRS, num_irs)
        fitness_sums_HC_IRS_RA = pool.map(HC_IRS_RA, num_irs)
        fitness_sums_RS = pool.map(RS, num_irs)
    ...
    plt.plot(num_irs, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    plt.plot(num_irs, fitness_sums_GA_IRS_RA, marker='s', linestyle='dotted',label = "C2G-A_IRS_RA")
    plt.plot(num_irs, fitness_sums_HC_IRS, marker='s', linestyle='-',label = "HC-IRS")
    plt.plot(num_irs, fitness_sums_HC_IRS_RA, marker='*', linestyle='-',label = "HC-IRS_RA")
    plt.plot(num_irs, fitness_sums_RS, marker='o', linestyle='dashdot',label = "RS")
