...
def GA_IRS(p_mutation): # Define function to process each p_max value
    print(f"calculaiton for p_mutation for GA_IRS",p_mutation)
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    p_mutation_values = np.arange(0.1,1,0.1) # P_max values from 1 to 11
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA_IRS = pool.map(GA_IRS, p_mutation_values)

        

    data_dict = {
        "Mutation_value": list(range(1, len(fitness_sums_GA_IRS) + 1)) if fitness_sums_GA_IRS else [], # Assuming generation number starts from 1
    ...
    p_mutation_values =np.arange(0.1,1,0.1)
    plt.rcParams["font.size"] = "20"
    plt.plot(p_mutation_values, fitness_sums_GA_IRS, marker='*', linestyle='dotted',label = "C2G-A_IRS")
    plt.xlabel('p_mutation',size=22)
    plt.ylabel('Energy',size=22)
    plt.legend()
    plt.savefig("Energy vs p_mutation.pdf", format="pdf", bbox_inches="tight", dpi=800) # saved with different name
    plt.show()
